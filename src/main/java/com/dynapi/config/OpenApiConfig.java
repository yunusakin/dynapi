...
public class OpenApiConfig {
  ...
  public OpenAPI customOpenAPI() {
    return new OpenAPI().info(new Info().title("Dynapi API").version("1.0").description("""
      ...
      """)).components(new Components().addExamples(EXAMPLE_VALIDATION_ERROR, new Example().value(Map.of("type", "about:blank", "title", "Validation Error", "status", 400, "detail", "Validation failed", "errors", Map.of("field", "must not be blank")))).addExamples(EXAMPLE_UNSUPPORTED_VERSION, new Example().value(Map.of("type", "about:blank", "title", "Bad Request", "status", 400, "detail", "Invalid API version '2'."))));
  }
  ...
  public OperationCustomizer apiVersionQueryParameterCustomizer() {
    return (operation, handlerMethod) -> {
      ...
      boolean hasVersionParam = operation.getParameters().stream().anyMatch(parameter -> API_VERSION_PARAM.equals(parameter.getName()));
      if (!hasVersionParam) {
  ...
  private Parameter apiVersionParameter() {
    ...
    return new Parameter().name(API_VERSION_PARAM).in("query").required(false).description("API version. Supported value: `1`.").schema(versionSchema);
  }

  private io.swagger.v3.oas.models.responses.ApiResponse badRequestResponse() {
    return new io.swagger.v3.oas.models.responses.ApiResponse().description("Validation, guardrail, or API version error.").content(new Content().addMediaType("application/problem+json", new MediaType().addExamples("validationError", new Example().$ref("#/components/examples/" + EXAMPLE_VALIDATION_ERROR)).addExamples("unsupportedApiVersion", new Example().$ref("#/components/examples/" + EXAMPLE_UNSUPPORTED_VERSION))));
  }
}
