...
public class SchemaVersionServiceImpl implements SchemaVersionService {
    ...
    public SchemaVersion getActiveVersion(String entityName) {
        Query query =
                new Query(
                        Criteria.where("entityName")
                                .is(entityName)
                                .and("status")
                                .is(SchemaLifecycleStatus.PUBLISHED));
        return mongoTemplate.findOne(query, SchemaVersion.class);
    }

    @Override
    public SchemaVersion getVersion(String entityName, Integer version) {
        Query query = new Query(Criteria.where("entityName").is(entityName).and("version").is(version));
        return mongoTemplate.findOne(query, SchemaVersion.class);
    }
    ...
    public void activateVersion(String entityName, Integer version) {
        ...
        Update deactivateUpdate =
                new Update()
                        .set("status", SchemaLifecycleStatus.DEPRECATED)
                        .set("deprecatedAt", LocalDateTime.now());
        mongoTemplate.updateFirst(activeQuery, deactivateUpdate, SchemaVersion.class);

        // Activate new version
        Query newVersionQuery =
                new Query(Criteria.where("entityName").is(entityName).and("version").is(version));
        Update activateUpdate =
                new Update()
                        .set("status", SchemaLifecycleStatus.PUBLISHED)
    ...
    public Map<String, Object> migrateData(
            Map<String, Object> data, Integer fromVersion, Integer toVersion) {
