...
public class AuditEventListener {
    ...
    public void handleAuditEvent(AuditEvent event) {
        logger.info("AUDIT | action={} | entity={} | details={} | timestamp={}",
                event.getAction(), event.getEntity(), event.getDetails(), event.getTimestamp());
        // Persist audit logs to MongoDB
        mongoTemplate.save(new AuditLog(event.getAction(), event.getEntity(), event.getDetails(), event.getTimestamp()), "audit_logs");
    }
