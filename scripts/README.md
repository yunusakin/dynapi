...
| `check-demo-boundary.sh` | Enforces strict public/demo boundary by failing on pro-only endpoint/capability markers in `src/main/java` | Every push/PR (runs in CI) |
...
3. `bash scripts/check-demo-boundary.sh`
4. `bash scripts/resolve-skills.sh --task-type api-change` (smoke check)
